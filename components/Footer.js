  ...
  return (
    <footer className="mt-20 border-t border-border bg-[#f4f0e7]">
      <div className="container-shell grid gap-10 py-12 md:grid-cols-[1.4fr_1fr_1fr]">
        <div><p className="text-xl font-black tracking-[-0.04em]">GharKa<span className="text-accent">Bite</span></p><p className="mt-3 max-w-sm text-sm leading-6 text-text-secondary">Everyday food cooked with the care, balance and familiarity of a home kitchen.</p><div className="mt-5 space-y-2 text-sm font-bold text-text-secondary"><p className="flex items-center gap-2"><MapPin className="size-4 text-primary" aria-hidden="true" /> Delivering within about 5 km</p><p className="flex items-center gap-2"><Clock3 className="size-4 text-primary" aria-hidden="true" /> Lunch & dinner, subject to availability</p></div></div>
