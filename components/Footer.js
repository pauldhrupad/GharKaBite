  ...
  return (
    <footer className="mt-8 border-t border-primary-hover bg-primary pb-[calc(5rem+env(safe-area-inset-bottom))] text-surface md:mt-12 lg:pb-0">
      <div className="container-shell grid grid-cols-2 gap-x-5 gap-y-6 py-7 md:grid-cols-[1.4fr_1fr_1fr] md:gap-10 md:py-10">
        <div className="col-span-2 md:col-span-1"><span className="inline-flex rounded-xl bg-surface p-1"><BrandMark className="size-14 md:size-16" /></span><p className="mt-2 max-w-sm text-sm leading-5 text-disabled-bg md:mt-3 md:leading-6">Everyday home-cooked food, delivered locally.</p><div className="mt-3 flex flex-wrap gap-x-4 gap-y-1 text-xs font-bold text-disabled-bg md:mt-4 md:block md:space-y-2 md:text-sm"><p className="flex items-center gap-2"><MapPin className="size-4 shrink-0" aria-hidden="true" /> Delivery within 3 km</p><p className="flex items-center gap-2"><Clock3 className="size-4 shrink-0" aria-hidden="true" /> Lunch & dinner</p></div></div>
