...
export default function DeliveryChecker() {
  ...
  return <div className="mt-7 rounded-2xl border border-border bg-surface p-4 shadow-[0_16px_40px_rgba(56,45,31,0.08)] sm:p-5">
    ...
    <form onSubmit={check} className="mt-5 flex flex-col gap-3 sm:flex-row">
      <label className="group relative min-w-0 flex-1"><span className="sr-only">Full delivery address</span><Search className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-text-secondary group-focus-within:text-primary" aria-hidden="true" /><input type="search" name="deliveryAddress" value={query} onChange={(event) => { setQuery(event.target.value); setSelected(null); setResult(null); }} className="input-field with-leading-icon" placeholder="Enter delivery address" autoComplete="street-address" enterKeyHint="search" /></label>
      <button disabled={busy || (!selected && query.trim().length < 10)} aria-busy={busy} className="min-h-12 w-full rounded-xl bg-primary px-4 text-sm font-black text-white disabled:opacity-50 sm:w-auto">{busy ? "Checking…" : "Check address"}</button>
    </form>
    ...
    <div className="mt-3"><LocationPicker compactTrigger location={selected?.location} onSelect={(choice) => { setSelected(choice); setQuery(choice.result.label || [choice.address.street, choice.address.area, choice.address.city, choice.address.pinCode].join(", ")); setSuggestions([]); setResult(choice.result); }} /></div>
