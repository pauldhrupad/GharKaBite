...
export default function LocationPicker({ location, onSelect }) {
  ...
  return <>
    ...
    {open && <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 p-2 sm:p-5" onMouseDown={(event) => { if (event.target === event.currentTarget) setOpen(false); }}><section role="dialog" aria-modal="true" aria-label="Choose delivery location" className="flex max-h-[95dvh] w-full max-w-2xl flex-col overflow-hidden rounded-2xl bg-surface shadow-2xl"><div className="flex items-center justify-between border-b border-border p-4"><div><h2 className="text-lg font-black">Choose your delivery point</h2><p className="text-xs text-text-secondary">Tap the map or drag the pin to the exact entrance.</p></div><button type="button" onClick={() => setOpen(false)} aria-label="Close map" className="rounded-lg p-2"><X className="size-5" /></button></div>
