...
export default function LocationPicker({ location, onSelect }) {
  ...
  return <>
    ...
    {open && <div className="ui-modal-backdrop fixed inset-0 z-[100] flex items-center justify-center bg-black/60 p-2 sm:p-5" onMouseDown={(event) => { if (event.target === event.currentTarget) setOpen(false); }}><section ref={dialogRef} role="dialog" aria-modal="true" aria-label="Choose delivery location" className="ui-dialog flex max-h-[95dvh] w-full max-w-2xl flex-col overflow-hidden rounded-2xl bg-surface shadow-2xl"><div className="flex items-center justify-between border-b border-border p-4"><div><h2 className="text-lg font-black">Choose your delivery point</h2><p className="text-xs text-text-secondary">Tap the map or drag the pin to the exact entrance.</p></div><button type="button" onClick={() => setOpen(false)} aria-label="Close map" className="grid size-11 place-items-center rounded-lg hover:bg-surface-muted"><X className="size-5" /></button></div>
