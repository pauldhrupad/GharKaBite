...
import { LogOut, MapPin, Plus, Save, Trash2, UserRound } from "lucide-react";
...
export default function ProfileForm({ initialUser }) {
  ...
  function updateAddress(index, event) {
    ...
    setProfile((current) => ({ ...current, addresses: current.addresses.map((address, addressIndex) => addressIndex === index ? { ...address, [name]: value } : address) }));
  }
  ...
  return (
    <form onSubmit={saveProfile} className="container-shell grid gap-6 py-10 lg:grid-cols-[20rem_1fr] lg:items-start">
      <aside className="card-surface p-5 lg:sticky lg:top-24"><span className="grid size-14 place-items-center rounded-2xl bg-primary/10 text-primary"><UserRound className="size-7" aria-hidden="true" /></span><h2 className="mt-4 text-xl font-black">{profile.name || "Your profile"}</h2><p className="mt-1 text-sm text-text-secondary">{profile.email}</p><div className="mt-5 border-t border-border pt-5"><button type="button" onClick={() => signOut({ callbackUrl: "/" })} className="inline-flex min-h-10 items-center gap-2 text-sm font-black text-danger"><LogOut className="size-4" aria-hidden="true" /> Sign out</button></div></aside>

      <div className="space-y-6">
        <section className="card-surface p-5 md:p-7"><h2 className="flex items-center gap-2 text-xl font-black"><UserRound className="size-5 text-primary" aria-hidden="true" /> Account details</h2><div className="mt-5 grid gap-4 sm:grid-cols-2"><label className="text-sm font-bold">Name<input className="input-field mt-2" name="name" value={profile.name} onChange={updateProfile} required /></label><label className="text-sm font-bold">Phone<input className="input-field mt-2" name="phone" value={profile.phone || ""} onChange={updateProfile} inputMode="numeric" maxLength={10} required /></label><label className="text-sm font-bold sm:col-span-2">Email<input className="input-field mt-2 bg-surface-muted" value={profile.email || ""} readOnly aria-describedby="email-note" /></label></div><p id="email-note" className="mt-2 text-xs text-text-secondary">Email changes are disabled to protect your login identity.</p></section>
        ...
        <section className="card-surface p-5 md:p-7"><div className="flex items-center justify-between gap-3"><div><h2 className="flex items-center gap-2 text-xl font-black"><MapPin className="size-5 text-primary" aria-hidden="true" /> Saved addresses</h2><p className="mt-1 text-sm text-text-secondary">Save up to five local delivery locations.</p></div><button type="button" onClick={addAddress} disabled={profile.addresses.length >= 5} className="inline-flex min-h-10 items-center gap-2 rounded-xl border border-border px-3 text-sm font-black text-primary disabled:opacity-40"><Plus className="size-4" aria-hidden="true" /> Add</button></div>
          {profile.addresses.length === 0 ? <div className="mt-5 rounded-xl border border-dashed border-border bg-surface-muted p-6 text-center text-sm text-text-secondary">No saved addresses yet.</div> : <div className="mt-5 space-y-4">{profile.addresses.map((address, index) => <fieldset key={address._id || index} className="rounded-xl border border-border p-4"><legend className="px-2 text-sm font-black">Address {index + 1}</legend><div className="grid gap-3 sm:grid-cols-2"><AddressInput label="Label" name="label" value={address.label || "Home"} onChange={(event) => updateAddress(index, event)} /><AddressInput label="House / Flat" name="house" value={address.house || ""} onChange={(event) => updateAddress(index, event)} /><AddressInput label="Street" name="street" value={address.street || ""} onChange={(event) => updateAddress(index, event)} /><AddressInput label="Area" name="area" value={address.area || ""} onChange={(event) => updateAddress(index, event)} /><AddressInput label="Landmark" name="landmark" value={address.landmark || ""} onChange={(event) => updateAddress(index, event)} optional /><AddressInput label="City" name="city" value={address.city || "Kolkata"} onChange={(event) => updateAddress(index, event)} /><AddressInput label="PIN Code" name="pinCode" value={address.pinCode || ""} onChange={(event) => updateAddress(index, event)} inputMode="numeric" maxLength={6} /></div><button type="button" onClick={() => removeAddress(index)} className="mt-3 inline-flex min-h-9 items-center gap-2 text-xs font-black text-danger"><Trash2 className="size-3.5" aria-hidden="true" /> Remove address</button></fieldset>)}</div>}
        </section>
...
function AddressInput({ label, optional, ...props }) {
  return <label className="text-xs font-bold">{label} {optional && <span className="font-normal text-text-secondary">(optional)</span>}<input className="input-field mt-1.5" {...props} /></label>;
}
