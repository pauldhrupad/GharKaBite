...
export default function ThaliCustomizer({ meal, serviceDate, editKey = "", restore = false }) {
  ...
  return <div className="mt-7 space-y-6 border-t border-border pt-6">
    <fieldset><legend className="text-sm font-black">Delivery period</legend><div className="mt-3 inline-flex rounded-xl border border-border bg-surface-muted p-1">{meal.slots.map((slot) => <label key={slot} className={`flex min-h-11 cursor-pointer items-center rounded-lg px-5 py-2.5 text-sm font-extrabold transition-colors focus-within:outline-2 focus-within:outline-primary active:scale-[0.99] ${period === slot ? "bg-surface text-primary shadow-sm" : "text-text-secondary hover:bg-white/60 hover:text-primary"}`}><input type="radio" name="thaliPeriod" checked={period === slot} onChange={() => setPeriod(slot)} className="sr-only" />{slot}</label>)}</div></fieldset>
    ...
    <div className="flex items-center justify-between gap-4"><p className="text-sm font-bold">Thali quantity</p><div className="flex items-center rounded-xl border border-border bg-surface"><button type="button" disabled={quantity <= 1} onClick={() => { setQuantity(Math.max(1, quantity - 1)); setAdded(false); }} aria-label="Decrease Thali quantity" className="grid size-11 place-items-center disabled:opacity-35"><Minus className="size-4" /></button><span className="min-w-8 text-center font-black" aria-live="polite">{quantity}</span><button type="button" disabled={quantity >= Math.min(10, meal.stock)} onClick={() => { setQuantity(Math.min(10, meal.stock, quantity + 1)); setAdded(false); }} aria-label="Increase Thali quantity" className="grid size-11 place-items-center disabled:opacity-35"><Plus className="size-4" /></button></div></div>
    {error && <p role="alert" className="rounded-xl bg-danger/8 p-3 text-sm font-bold text-danger">{error}</p>}
    <div className="sticky bottom-20 z-20 -mx-4 border-t border-border bg-background/95 p-4 backdrop-blur-sm sm:static sm:mx-0 sm:border-0 sm:bg-transparent sm:p-0"><button type="button" onClick={addToCart} disabled={soldOut || !availability.available} className="inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-xl bg-primary px-5 text-sm font-extrabold text-white disabled:cursor-not-allowed disabled:opacity-50">{added ? <Check className="size-4" aria-hidden="true" /> : <ShoppingBag className="size-4" aria-hidden="true" />}{soldOut ? "Sold Out" : !availability.available ? "Ordering Closed" : added ? "Added to cart" : `${editKey ? "Save customization" : "Add customized Thali"} · ₹${unitTotal * quantity}`}</button><span className="sr-only" role="status" aria-live="polite">{added ? `${meal.name} added to cart.` : ""}</span></div>
  </div>;
}
