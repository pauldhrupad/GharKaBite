...
export default function ThaliCustomizer({ meal, serviceDate, editKey = "", restore = false }) {
  ...
  return <div className="mt-7 space-y-6 border-t border-border pt-6">
    ...
    <div className="sticky bottom-20 z-20 -mx-4 border-t border-border bg-background/95 p-4 backdrop-blur-sm sm:static sm:mx-0 sm:border-0 sm:bg-transparent sm:p-0"><button type="button" onClick={addToCart} disabled={unavailable || !availability.available} className="inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-xl bg-primary px-5 text-sm font-extrabold text-white disabled:cursor-not-allowed disabled:opacity-50">{added ? <Check className="size-4" aria-hidden="true" /> : <ShoppingBag className="size-4" aria-hidden="true" />}{unavailable ? "Currently unavailable" : !availability.available ? "Ordering Closed" : added ? "Added to cart" : `${editKey ? "Save customization" : meal.kind === "dish" ? "Add dish to cart" : "Add customized Thali"} · ₹${unitTotal * quantity}`}</button><span className="sr-only" role="status" aria-live="polite">{added ? `${meal.name} added to cart.` : ""}</span></div>
  </div>;
}
