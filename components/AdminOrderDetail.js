...
export default function AdminOrderDetail({ orderId, initialOrder }) {
  ...
  const cashDialogRef = useModalFocus(confirmingPayment, () => { if (!savingPayment) setConfirmingPayment(false); });
  const verificationDialogRef = useModalFocus(Boolean(verificationAction), () => { if (!savingPayment) setVerificationAction(""); });
  ...
  return (
    <>
      ...
      {confirmingPayment && <div className="ui-modal-backdrop fixed inset-0 z-50 grid place-items-center bg-black/60 p-4" onPointerDown={(event) => dismissOnBackdrop(event, () => { if (!savingPayment) setConfirmingPayment(false); })}><section ref={cashDialogRef} role="alertdialog" aria-modal="true" aria-labelledby="cash-confirm-title" aria-describedby="cash-confirm-description" className="ui-dialog w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl"><h2 id="cash-confirm-title" className="text-xl font-black">Confirm cash received?</h2><p id="cash-confirm-description" className="mt-3 text-sm leading-6 text-text-secondary">Only mark order {order.orderNumber} as paid if you have collected ₹{order.total} in cash. This will complete the order.</p>{paymentMessage && <p className="mt-3 text-sm font-bold text-danger" role="alert">{paymentMessage}</p>}<div className="mt-6 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end"><button type="button" autoFocus onClick={() => setConfirmingPayment(false)} disabled={savingPayment} className="min-h-11 rounded-xl border border-border px-4 text-sm font-black disabled:opacity-50">Cancel</button><button type="button" onClick={confirmCashReceived} disabled={savingPayment} aria-busy={savingPayment} className="min-h-11 rounded-xl bg-primary px-4 text-sm font-black text-white disabled:opacity-50">{savingPayment ? "Saving…" : `Yes, ₹${order.total} received`}</button></div></section></div>}
      {verificationAction && <div className="ui-modal-backdrop fixed inset-0 z-50 grid place-items-center bg-black/60 p-4" onPointerDown={(event) => dismissOnBackdrop(event, () => { if (!savingPayment) setVerificationAction(""); })}><section ref={verificationDialogRef} role="alertdialog" aria-modal="true" aria-label={verificationAction === "confirm" ? "Confirm payment" : "Reject payment"} className="ui-dialog w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl"><h2 className="text-xl font-black">{verificationAction === "confirm" ? "Confirm Payment?" : "Reject Payment?"}</h2><p className="mt-3 text-sm leading-6 text-text-secondary">{verificationAction === "confirm" ? `Confirm that ₹${order.total} has been received for Order ${order.orderNumber}? Check your bank or UPI account first.` : `Reject payment proof for ${order.orderNumber}? The customer can resubmit.`}</p>{verificationAction === "reject" && <label className="mt-4 block text-sm font-bold">Reason<CustomSelect value={rejectionReason} onChange={(event) => setRejectionReason(event.target.value)} className="input-field mt-2">{["Transaction not found", "Wrong amount paid", "Invalid payment proof", "Screenshot unclear", "Duplicate transaction ID", "Other"].map((reason) => <option key={reason}>{reason}</option>)}</CustomSelect></label>}{paymentMessage && <p role="alert" className="mt-3 text-sm font-bold text-danger">{paymentMessage}</p>}<div className="mt-6 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end"><button type="button" autoFocus onClick={() => setVerificationAction("")} disabled={savingPayment} className="min-h-11 rounded-xl border border-border px-4 text-sm font-black">Cancel</button><button type="button" onClick={verifyOnlinePayment} disabled={savingPayment} aria-busy={savingPayment} className="min-h-11 rounded-xl bg-primary px-4 text-sm font-black text-white disabled:opacity-50">{savingPayment ? "Saving…" : verificationAction === "confirm" ? "Confirm Payment" : "Reject Payment"}</button></div></section></div>}
    </>
  );
}
