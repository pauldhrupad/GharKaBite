  ...
  return <>
    ...
    <aside className={`hidden shrink-0 bg-[#173c27] text-white transition-[width] duration-200 lg:block ${collapsed ? "w-20" : "w-64"}`}>
      <div className={`flex h-17 items-center border-b border-white/10 ${collapsed ? "justify-center" : "justify-between px-4"}`}>
        {!collapsed && <Link href="/admin/dashboard" className="flex items-center" aria-label="GharKaBite admin dashboard"><BrandMark className="size-14 shrink-0 rounded-md bg-white" /></Link>}
    ...
    {open && <div className="fixed inset-0 z-50 lg:hidden"><button type="button" className="absolute inset-0 bg-black/45" onClick={() => setOpen(false)} aria-label="Close admin menu overlay" /><aside className="relative h-full w-[min(82vw,18rem)] bg-[#173c27] text-white shadow-2xl"><div className="flex h-17 items-center justify-between border-b border-white/10 px-5"><Link href="/admin/dashboard" onClick={() => setOpen(false)} className="flex items-center" aria-label="GharKaBite admin dashboard"><BrandMark className="size-14 shrink-0 rounded-md bg-white" /></Link><button type="button" className="grid size-9 place-items-center rounded-lg hover:bg-white/10" onClick={() => setOpen(false)} aria-label="Close admin menu"><X className="size-5" /></button></div><Navigation pathname={pathname} onNavigate={() => setOpen(false)} /></aside></div>}
  </>;
}
