...
export default function AdminSidebar() {
  ...
  const nav = <><div className="flex h-17 items-center justify-between border-b border-white/10 px-5"><Link href="/admin/dashboard" className="text-lg font-black">GharKa<span className="text-[#eda585]">Bite</span> <span className="text-xs font-bold text-white/50">ADMIN</span></Link><button className="grid size-9 place-items-center rounded-lg hover:bg-white/10 lg:hidden" onClick={() => setOpen(false)} aria-label="Close admin menu"><X className="size-5" /></button></div><nav className="p-3" aria-label="Admin navigation">{links.map(({ href,label,icon:Icon }) => { const active = pathname.startsWith(href); return <Link key={href} href={href} onClick={() => setOpen(false)} className={`mb-1 flex items-center gap-3 rounded-xl px-3 py-3 text-sm font-extrabold transition ${active ? "bg-white text-primary" : "text-white/68 hover:bg-white/8 hover:text-white"}`}><Icon className="size-5" aria-hidden="true" />{label}</Link>; })}</nav></>;
