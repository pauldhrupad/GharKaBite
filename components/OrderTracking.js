  ...
  return (
    <main className="container-shell py-10 md:py-14">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        ...
        <div className="flex items-center gap-2"><Badge tone={isCancelled ? "terracotta" : "green"}>{isCancelled ? "Cancelled" : orderStatuses[currentIndex]?.label || "Received"}</Badge>{order.paymentMethod === "DEMO" && <Badge>Demo payment</Badge>}</div>
      </div>
      ...
      <section className="card-surface mt-8 overflow-hidden">
        <div className="border-b border-border bg-[linear-gradient(120deg,var(--primary),var(--primary-hover))] px-5 py-6 text-white md:px-8">
          <div className="flex items-center gap-3"><span className="grid size-11 place-items-center rounded-xl bg-white/12"><ChefHat className="size-5" aria-hidden="true" /></span><div><h2 className="text-xl font-black">{isCancelled ? "This order was cancelled" : "Your meal is in progress"}</h2><p className="mt-1 text-sm text-white/75">{order.serviceDate || "Today"} · {order.mealPeriod} · {order.deliverySlot}</p></div></div>
        </div>
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[1fr_22rem]">
        ...
        <aside className="space-y-5">
          ...
          <section className="card-surface p-5"><h2 className="flex items-center gap-2 font-black"><Clock3 className="size-4 text-primary" aria-hidden="true" /> Payment & total</h2><div className="mt-3 space-y-2 text-sm"><div className="flex justify-between text-text-secondary"><span>{order.paymentMethod === "COD" ? "Cash on Delivery" : "Demo payment"}</span><span className="capitalize">{order.paymentStatus}</span></div><div className="flex justify-between border-t border-border pt-3 text-lg font-black"><span>Total</span><span>₹{order.total}</span></div></div></section>
        </aside>
      </div>
