    ...
    <section className="container-shell py-10">
      ...
      <div className="space-y-5">{orders.map((order) => (
        <article key={order.orderNumber} className="card-surface overflow-hidden">
          ...
          <div className="grid gap-5 p-5 md:grid-cols-[1fr_auto] md:items-end"><div><p className="flex items-center gap-2 text-xs font-extrabold uppercase tracking-wide text-text-secondary"><UtensilsCrossed className="size-4" aria-hidden="true" /> {order.mealPeriod} · {order.deliverySlot}</p><p className="mt-3 font-black">{order.items.map((item) => `${item.quantity}× ${item.name}`).join(" · ")}</p><p className="mt-2 text-sm text-text-secondary">Total ₹{order.total} · {order.paymentMethod === "COD" ? "Cash on Delivery" : "Demo payment"}</p></div><div className="flex flex-wrap gap-2"><Button type="button" variant="secondary" onClick={() => reorder(order)}><RotateCcw className="size-4" aria-hidden="true" /> Order Again</Button><Button href={`/orders/${order.orderNumber}`}>Track <ArrowRight className="size-4" aria-hidden="true" /></Button></div></div>
        </article>
      ))}</div>
    </section>
  );
