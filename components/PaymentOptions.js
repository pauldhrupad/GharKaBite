...
export default function PaymentOptions({ payment, channel, onChange, amount, orderNumber }) {
  ...
  async function copy(value, key) {
    try { await navigator.clipboard.writeText(value); setCopied(key); window.setTimeout(() => setCopied(""), 2500); }
    catch { setCopied(""); }
  }
  ...
  return <div><h3 className="text-lg font-black">Choose how you want to pay</h3><p className="mt-1 text-sm leading-6 text-text-secondary">Pay using the details below. Your order will be confirmed after payment verification.</p>
    <div className="mt-4 grid gap-2 sm:grid-cols-3">{channels.map((item) => { const Icon = icons[item]; return <button type="button" key={item} onClick={() => onChange(item)} className={`flex min-h-12 items-center justify-center gap-2 rounded-xl border px-3 text-sm font-black ${channel === item ? "border-primary bg-primary/8 text-primary" : "border-border"}`} aria-pressed={channel === item}><Icon className="size-4" />{labels[item]}</button>; })}</div>
