  ...
  return (
    <section className="mt-7 overflow-hidden rounded-2xl border border-border bg-white">
      <div className="space-y-3 border-b border-border p-4 sm:p-5">
        <label className="group relative block min-w-0"><Search className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-text-secondary group-focus-within:text-primary" aria-hidden="true" /><span className="sr-only">Search by order ID, customer or phone</span><input type="search" name="orderSearch" value={query} onChange={(event) => setQuery(event.target.value)} className={`input-field with-leading-icon ${query ? "pr-12" : ""}`} placeholder="Search orders or customers" autoComplete="off" enterKeyHint="search" />{query && <button type="button" onClick={() => setQuery("")} aria-label="Clear order search" className="absolute right-1 top-1/2 grid size-10 -translate-y-1/2 place-items-center rounded-lg text-text-secondary hover:bg-surface-muted hover:text-primary"><X className="size-4" aria-hidden="true" /></button>}</label>
        ...
        {filtersOpen && <div id="order-advanced-filters" className="ui-enter grid gap-3 rounded-xl border border-border bg-surface-muted/50 p-3 sm:grid-cols-2 sm:p-4">
