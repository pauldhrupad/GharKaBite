export default function ThaliOrderDetails({ item }) {
  ...
  return <div className="mt-2 space-y-1 text-xs leading-5 text-text-secondary">
    {fixed.length > 0 && item.kind !== "dish" && <p><span className="font-bold">Included:</span> {fixed.map((entry) => entry.name).join(", ")}</p>}
    ...
    {addOns.length > 0 && <p><span className="font-bold">Extras per {item.kind === "dish" ? "dish" : "Thali"}:</span> {addOns.map((entry) => `${entry.name} ×${entry.quantity} (+₹${entry.price * entry.quantity})`).join(", ")}</p>}
