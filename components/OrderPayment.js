  ...
  return <main className="container-shell max-w-5xl py-7 md:py-10"><p className="eyebrow">Online Payment</p><div className="mt-2 flex flex-wrap items-end justify-between gap-4"><div><h1 className="text-3xl font-black">Complete Your Payment</h1><p className="mt-2 break-all text-sm text-text-secondary">Order {order.orderNumber}</p></div><p className="text-4xl font-black text-accent">₹{Number(order.total).toLocaleString("en-IN")}</p></div>
    ...
    <div className="mt-6 grid gap-6 lg:grid-cols-[1fr_20rem]"><div className="space-y-6">
      {order.paymentStatus === "paid" ? <section className="card-surface p-6"><CheckCircle2 className="size-8 text-success" /><h2 className="mt-3 text-xl font-black">Payment Verified ✓</h2><p className="mt-2 text-sm text-text-secondary">Your payment has been verified and your order is confirmed.</p><Link href={`/orders/${orderId}`} className="mt-4 inline-flex font-black text-primary underline">Track your order</Link></section> : order.paymentStatus === "verification_pending" ? <section className="card-surface p-6"><Clock3 className="size-8 text-warning" /><h2 className="mt-3 text-xl font-black">Payment Submitted for Verification</h2><p className="mt-2 text-sm text-text-secondary">We&apos;ve received your payment details. Your order will be confirmed after verification.</p><p className="mt-3 text-sm font-bold">Proof: {order.paymentProofMethod === "whatsapp" ? "WhatsApp" : "Website"}{order.paymentReference ? ` · UTR ${order.paymentReference}` : ""}</p>{order.paymentProofMethod === "website" && <a href={`/api/orders/${encodeURIComponent(orderId)}/payment-screenshot`} target="_blank" rel="noopener noreferrer" className="mt-3 inline-block font-bold text-primary underline">View submitted screenshot</a>}</section> : order.orderStatus === "cancelled" ? <section className="card-surface p-6"><h2 className="text-xl font-black">Order cancelled</h2><p className="mt-2 text-sm text-text-secondary">Contact us if you need assistance with a payment already sent.</p></section> : <>
        ...
        <section className="card-surface p-5 md:p-7"><h2 className="text-xl font-black">Submit Payment Details</h2><p className="mt-2 text-sm text-text-secondary">After paying, keep your screenshot and Transaction ID / UTR. Your order stays payment pending until we verify it.</p><button type="button" onClick={() => setShowProof((current) => !current)} aria-expanded={showProof} aria-controls="payment-proof-form" className="mt-4 min-h-11 rounded-xl bg-primary px-5 text-sm font-black text-white">{showProof ? "Hide payment form" : order.paymentStatus === "rejected" ? "Resubmit Payment Details" : "I Have Paid · Send Payment Proof"}</button>{showProof && <form id="payment-proof-form" onSubmit={submitWebsite} className="ui-enter mt-5 space-y-4 rounded-xl border border-border p-4"><label className="block text-sm font-bold">Transaction ID / UTR<input required minLength={6} maxLength={50} value={reference} onChange={(event) => setReference(event.target.value)} className="input-field mt-2" placeholder="Enter the ID shown in your payment app" /></label><PaymentScreenshotPicker file={screenshot} onChange={setScreenshot} disabled={busy} /><label className="block text-sm font-bold">Optional Note<textarea value={note} maxLength={500} onChange={(event) => setNote(event.target.value)} className="input-field mt-2 min-h-20" /></label><button disabled={busy} aria-busy={busy} className="flex min-h-11 items-center gap-2 rounded-xl bg-primary px-5 text-sm font-black text-white disabled:opacity-50"><Upload className="size-4" aria-hidden="true" />{busy ? "Submitting…" : "Submit for verification"}</button></form>}</section>
