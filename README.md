...
- `/admin/meals` is the menu builder. The owner can edit included items, single-choice OR groups, multi-choice groups, option prices, preparation quantities, add-ons, and daily availability. Orders are made to order, not limited by meal stock. Archived items disappear from future menus while past order snapshots remain intact.
...
Run `npm run lint`, `npm run build`, and `npm test`. With provider credentials and an isolated database, exercise the menu builder, required OR choices, multi-choice limits, optional add-ons, variant cart/edit, checkout snapshots, subscription base-only coverage, reorder, homepage delivery checks, login, COD, all three UPI methods, QR upload, screenshot proof, WhatsApp proof, rejection and resubmission, admin verification, order tracking, ordering cutoffs, preparation totals and mobile layouts. Verify idempotency and server-side cutoff enforcement, not just UI labels.
