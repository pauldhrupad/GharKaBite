...
export default function AdminMealsPage() {
  ...
  return <section>
    ...
    <div className="mt-6 grid gap-4 lg:grid-cols-2">{meals.map((meal) => {
      ...
      return <article key={meal.slug} className="overflow-hidden rounded-2xl border border-border bg-white"><div className="flex gap-4 p-4"><div className="relative size-24 shrink-0 overflow-hidden rounded-xl"><Image src={meal.image} alt="" fill sizes="96px" className="object-cover" /></div><div><h2 className="font-black">{meal.name}</h2><p className="text-sm text-text-secondary">₹{meal.price} · {meal.category}</p><p className="text-xs text-text-secondary">Remaining {override?.remaining ?? meal.stockLimit}/{meal.stockLimit}</p><p className="text-xs font-bold">{meal.active ? "Active" : "Inactive"}{override?.soldOut ? " · Sold out" : ""}</p></div></div><div className="grid gap-2 border-t border-border p-4 sm:grid-cols-2"><button disabled={busy} onClick={() => open(meal)} className="rounded-lg border border-border p-2 text-sm font-bold">Edit</button><button disabled={busy} onClick={() => change(`/api/admin/meals/${meal.slug}`, "PATCH", { active: !meal.active })} className="rounded-lg border border-border p-2 text-sm font-bold">{meal.active ? "Deactivate" : "Activate"}</button><select aria-label={`${meal.name} availability`} value={override?.availableOverride === null || override?.availableOverride === undefined ? "default" : String(override.availableOverride)} onChange={(event) => dayPayload({ availableOverride: event.target.value === "default" ? null : event.target.value === "true" })} className="input-field text-sm"><option value="default">Weekly schedule</option><option value="true">Available</option><option value="false">Unavailable</option></select><button disabled={busy} onClick={() => dayPayload({ soldOut: !override?.soldOut })} className="rounded-lg border border-border p-2 text-sm font-bold">{override?.soldOut ? "Unmark sold out" : "Mark sold out"}</button><label className="text-xs font-bold">Remaining stock<input key={`${date}-${override?.remaining}`} type="number" min="0" max={meal.stockLimit} defaultValue={override?.remaining ?? meal.stockLimit} onBlur={(event) => { const value = Number(event.target.value); if (value !== (override?.remaining ?? meal.stockLimit)) dayPayload({ remaining: value }); }} className="input-field mt-1" /></label><button disabled={busy} onClick={() => { if (window.confirm(`Delete ${meal.name}? Past orders will be kept.`)) change(`/api/admin/meals/${meal.slug}`, "DELETE"); }} className="rounded-lg border border-danger/30 p-2 text-sm font-bold text-danger">Delete…</button></div></article>;
    })}</div>
    {form && <div className="fixed inset-0 z-50 overflow-y-auto bg-black/50 p-4"><form onSubmit={save} className="mx-auto my-5 max-w-3xl space-y-4 rounded-2xl bg-white p-5 sm:p-7"><div className="flex justify-between"><h2 className="text-2xl font-black">{editing ? "Edit meal" : "Add meal"}</h2><button type="button" onClick={() => setForm(null)}>Close</button></div><div className="grid gap-4 sm:grid-cols-2">{[["name", "Meal name"], ["shortDescription", "Short description"], ["price", "Price (₹)"], ["stockLimit", "Daily stock limit"]].map(([field, label]) => <label key={field} className="text-sm font-bold">{label}<input required type={["price", "stockLimit"].includes(field) ? "number" : "text"} min={field === "price" ? 1 : 0} value={form[field]} onChange={(event) => set(field, event.target.value)} className="input-field mt-1" /></label>)}<label className="text-sm font-bold">Category<select value={form.category} onChange={(event) => set("category", event.target.value)} className="input-field mt-1">{["Veg", "Egg", "Chicken", "Fish"].map((value) => <option key={value}>{value}</option>)}</select></label><label className="text-sm font-bold">Meal type<select value={form.mealType} onChange={(event) => set("mealType", event.target.value)} className="input-field mt-1">{["Rice Meal", "Roti Meal", "Comfort Meal", "Light Meal"].map((value) => <option key={value}>{value}</option>)}</select></label></div><label className="block text-sm font-bold">Full description<textarea required value={form.description} onChange={(event) => set("description", event.target.value)} className="input-field mt-1 min-h-24" /></label><label className="block text-sm font-bold">Contents (one per line)<textarea required value={form.contents} onChange={(event) => set("contents", event.target.value)} className="input-field mt-1 min-h-24" /></label><fieldset><legend className="text-sm font-black">Meal periods</legend><div className="mt-2 flex gap-4">{["Lunch", "Dinner"].map((value) => <label key={value} className="text-sm"><input type="checkbox" checked={form.slots.includes(value)} onChange={() => toggle("slots", value)} /> {value}</label>)}</div></fieldset><fieldset><legend className="text-sm font-black">Available days</legend><div className="mt-2 flex flex-wrap gap-3">{days.map((label, value) => <label key={label} className="text-sm"><input type="checkbox" checked={form.availableDays.includes(value)} onChange={() => toggle("availableDays", value)} /> {label}</label>)}</div></fieldset><fieldset><legend className="text-sm font-black">Badges</legend><div className="mt-2 flex flex-wrap gap-3">{badges.map((value) => <label key={value} className="text-sm"><input type="checkbox" checked={form.badges.includes(value)} onChange={() => toggle("badges", value)} /> {value}</label>)}</div></fieldset><div className="flex gap-5"><label className="text-sm"><input type="checkbox" checked={form.active} onChange={(event) => set("active", event.target.checked)} /> Active</label><label className="text-sm"><input type="checkbox" checked={form.featured} onChange={(event) => set("featured", event.target.checked)} /> Featured</label></div><div className="grid gap-3 sm:grid-cols-2"><label className="text-sm font-bold">Local image<select value={images.includes(form.image) ? form.image : ""} onChange={(event) => { setForm({ ...form, image: event.target.value, imagePublicId: "" }); }} className="input-field mt-1"><option value="" disabled>Uploaded image</option>{images.map((image) => <option key={image} value={image}>{image.split("/").pop()}</option>)}</select></label><label className="text-sm font-bold">Or upload image (max 5 MB)<input type="file" accept="image/jpeg,image/png,image/webp" onChange={upload} className="mt-2 block w-full text-sm" /></label></div><div className="relative h-40 overflow-hidden rounded-xl bg-surface-muted"><Image src={form.image} alt="Meal preview" fill sizes="700px" className="object-cover" /></div><button disabled={busy} type="submit" className="rounded-xl bg-primary px-6 py-3 font-black text-white disabled:opacity-50">{busy ? "Saving…" : "Save meal"}</button>{message && <p role="alert" className="text-sm text-danger">{message}</p>}</form></div>}
  </section>;
}
