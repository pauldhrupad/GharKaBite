...
export default function AdminMealsPage() {
  ...
  const reload = useCallback(async () => {
    ...
    if (!catalogue.ok || !overrides.ok) throw new Error("Unable to load Thalis.");
  ...
  async function change(url, method, payload) {
    ...
    try {
      ...
      await reload(); setMessage(result.message || "Thali updated.");
  ...
  return <section>
    <div className="flex flex-wrap items-end justify-between gap-4"><div><p className="eyebrow">Kitchen catalogue</p><h1 className="mt-1 text-3xl font-black">Thalis</h1><p className="text-sm text-text-secondary">Manage included items, OR choices, add-ons and daily stock.</p></div><button type="button" onClick={() => { setEditing(null); setForm({}); }} className="rounded-xl bg-primary px-4 py-3 text-sm font-black text-white">Create Thali</button></div>
    ...
    <div className="mt-6 grid gap-4 lg:grid-cols-2">{meals.map((meal) => {
      ...
      return <article key={meal.slug} className="overflow-hidden rounded-2xl border border-border bg-white"><div className="flex gap-4 p-4"><div className="relative size-24 shrink-0 overflow-hidden rounded-xl"><Image src={meal.image} alt="" fill sizes="96px" className="object-cover" /></div><div className="min-w-0"><h2 className="font-black">{meal.name}</h2><p className="text-sm text-text-secondary">From ₹{meal.price} · {meal.category}</p><p className="text-xs text-text-secondary">{meal.slots.join(" & ")} · Remaining {override?.remaining ?? meal.stockLimit}/{meal.stockLimit}</p><p className="text-xs font-bold">{meal.active ? "Active" : "Inactive"}{override?.soldOut ? " · Sold out" : ""}{meal.featured ? " · Featured" : ""}</p></div></div><div className="grid gap-2 border-t border-border p-4 sm:grid-cols-2"><button disabled={busy} onClick={() => { setEditing(meal.slug); setForm(meal); }} className="rounded-lg border border-border p-2 text-sm font-bold">Edit</button><button disabled={busy} onClick={() => duplicate(meal)} className="rounded-lg border border-border p-2 text-sm font-bold">Duplicate</button><button disabled={busy} onClick={() => change(`/api/admin/meals/${meal.slug}`, "PATCH", { active: !meal.active })} className="rounded-lg border border-border p-2 text-sm font-bold">{meal.active ? "Deactivate" : "Activate"}</button><button disabled={busy} onClick={() => dayPayload({ soldOut: !override?.soldOut })} className="rounded-lg border border-border p-2 text-sm font-bold">{override?.soldOut ? "Unmark sold out" : "Mark sold out"}</button><CustomSelect aria-label={`${meal.name} availability`} value={override?.availableOverride == null ? "default" : String(override.availableOverride)} onChange={(event) => dayPayload({ availableOverride: event.target.value === "default" ? null : event.target.value === "true" })} className="input-field text-sm"><option value="default">Weekly schedule</option><option value="true">Available</option><option value="false">Unavailable</option></CustomSelect><label className="text-xs font-bold">Remaining stock<input key={`${date}-${override?.remaining}`} type="number" min="0" max={meal.stockLimit} defaultValue={override?.remaining ?? meal.stockLimit} onBlur={(event) => { const value = Number(event.target.value); if (value !== (override?.remaining ?? meal.stockLimit)) dayPayload({ remaining: value }); }} className="input-field mt-1" /></label><button disabled={busy} onClick={() => { if (window.confirm(`Archive ${meal.name}? Past orders will be kept.`)) change(`/api/admin/meals/${meal.slug}`, "DELETE"); }} className="rounded-lg border border-danger/30 p-2 text-sm font-bold text-danger sm:col-span-2">Archive…</button></div></article>;
    })}</div>
    {form && <AdminThaliForm key={editing || form.slug || "new"} initial={Object.keys(form).length ? form : null} editing={editing} onClose={() => setForm(null)} onSaved={async () => { setForm(null); await reload(); setMessage("Thali saved."); }} />}
  </section>;
}
