  ...
  return (
    <>
      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-[0.75fr_1.25fr]">
        <section className="rounded-2xl border border-border bg-white p-5"><div className="flex items-center gap-3"><span className="grid size-10 place-items-center rounded-xl bg-accent/10 text-accent"><ChefHat className="size-5" aria-hidden="true" /></span><div><h2 className="font-black">Kitchen preparation summary</h2><p className="text-xs text-text-secondary">Accepted menu items and selected extras today</p></div></div><div className="mt-5 grid grid-cols-2 gap-3">{categories.map(({ category, quantity }) => <div key={category} className="rounded-xl bg-surface-muted p-4"><p className="text-2xl font-black">{quantity}</p><p className="text-sm font-bold text-text-secondary">{category} items</p></div>)}</div>{selectedCounts.size > 0 && <div className="mt-4"><h3 className="text-sm font-black">Customer choices</h3><div className="mt-2 flex flex-wrap gap-2">{[...selectedCounts].map(([name, count]) => <span key={name} className="rounded-full bg-primary/8 px-3 py-1 text-xs font-bold">{name} ×{count}</span>)}</div></div>}{addOnCounts.size > 0 && <div className="mt-4"><h3 className="text-sm font-black">Add-ons to prepare</h3><div className="mt-2 flex flex-wrap gap-2">{[...addOnCounts].map(([name, count]) => <span key={name} className="rounded-full bg-accent/8 px-3 py-1 text-xs font-bold">{name} ×{count}</span>)}</div></div>}</section>
