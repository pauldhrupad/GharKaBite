...
import MealDetailActions from "@/components/MealDetailActions";
...
export async function generateMetadata({ params }) {
  ...
  if (!meal) return { title: "Meal Not Found | GharKaBite" };
  return { title: `${meal.name} | GharKaBite`, description: meal.shortDescription };
}
    ...
    <>
      <section className="container-shell py-8 md:py-14">
        ...
        <div className="grid gap-8 lg:grid-cols-2 lg:items-start">
          <div className="relative aspect-[4/3] overflow-hidden rounded-[1.75rem] border border-border bg-surface-muted lg:sticky lg:top-24">
            <Image src={meal.image} alt={`${meal.name}: ${meal.contents.join(", ")}`} fill priority sizes="(max-width: 1024px) 100vw, 50vw" className={`object-cover ${soldOut ? "grayscale-[35%]" : ""}`} />
            {soldOut && <div className="absolute inset-0 grid place-items-center bg-text-primary/55"><span className="rounded-full bg-white px-5 py-2.5 font-black">Sold Out</span></div>}
          </div>
          ...
          <div className="lg:py-2">
            ...
            <div className="mt-7 grid gap-3 sm:grid-cols-3">
              <div className="rounded-xl bg-surface-muted p-4"><Layers3 className="size-5 text-primary" aria-hidden="true" /><p className="mt-2 text-xs font-bold text-text-secondary">Meal type</p><p className="font-black">{meal.mealType}</p></div>
            ...
            <div className="mt-6 rounded-2xl border border-border bg-surface p-5">
              <h2 className="text-sm font-black uppercase tracking-wide text-text-secondary">Meal contents</h2>
              <ul className="mt-4 grid gap-3 sm:grid-cols-2">
                {meal.contents.map((item) => <li key={item} className="flex items-center gap-2 text-sm font-bold"><span className="grid size-5 place-items-center rounded-full bg-success/10"><Check className="size-3 text-success" aria-hidden="true" /></span>{item}</li>)}
              </ul>
            </div>

            <MealDetailActions meal={meal} serviceDate={date} />
          </div>
        </div>
      </section>
