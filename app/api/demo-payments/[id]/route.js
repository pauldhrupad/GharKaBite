...
export async function POST(request, { params }) {
  ...
  try {
    ...
    if (!["success", "failure"].includes(outcome)) return Response.json({ message: "Choose a plan preview outcome." }, { status: 400 });
    await dbConnect();
    ...
    if (intent.kind === "order") return Response.json({ message: "This payment route is unavailable for orders." }, { status: 409 });
    if (intent.status === "failed") return Response.json({ message: "This plan preview failed. Start again." }, { status: 409 });
    ...
    if (intent.expiresAt < new Date()) return Response.json({ message: "Plan preview expired. Start again." }, { status: 409 });
    if (outcome === "failure") {
      const failed = await DemoPayment.findOneAndUpdate({ _id: id, status: "pending" }, { status: "failed" });
      if (!failed) return Response.json({ message: "This plan preview is already processing." }, { status: 409 });
      return Response.json({ message: "Plan preview failed. No plan was created.", status: "failed", demo: true });
    }
    ...
    if (!claimedIntent) return Response.json({ message: "This plan preview is already processing." }, { status: 409 });
    claimed = true;
    ...
    return Response.json({ message: error instanceof CheckoutError ? error.message : error.message === "RATE_LIMITED" ? "Too many attempts. Try later." : "Plan preview could not be completed." }, { status: error.status || (error.message === "RATE_LIMITED" ? 429 : 503) });
  }
}
