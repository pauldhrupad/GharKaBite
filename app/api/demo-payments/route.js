...
export async function POST(request) {
  ...
  try {
    ...
    if (kind !== "subscription" || !/^[0-9a-f-]{36}$/i.test(key || "") || !payload || typeof payload !== "object" || JSON.stringify(payload).length > 20000 || payload.purchaseKey !== key) return Response.json({ message: "This plan preview route is unavailable for food orders." }, { status: 400 });
    await dbConnect();
  ...
  } catch (error) { return Response.json({ message: error.message === "RATE_LIMITED" ? "Too many plan preview attempts. Try later." : "Unable to start the plan preview." }, { status: error.message === "RATE_LIMITED" ? 429 : 503 }); }
}
