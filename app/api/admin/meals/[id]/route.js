...
export async function PATCH(request, { params }) {
  ...
  try {
    ...
    if (!current) return Response.json({ message: "Menu item not found." }, { status: 404 });
    ...
    return Response.json({ message: "Unable to update menu item." }, { status: 503 });
  }
}
...
export async function DELETE(_request, { params }) {
  ...
  try {
    ...
    if (!meal) return Response.json({ message: "Menu item not found." }, { status: 404 });
    return Response.json({ message: "Menu item archived." });
  } catch { return Response.json({ message: "Unable to archive menu item." }, { status: 503 }); }
}
