...
export function normalizeMeal(input) {
  ...
  const fixedItems = kind === "dish" ? [{ name, description: "" }] : Array.isArray(input.fixedItems) ? input.fixedItems.map((item) => ({ name: plain(item.name, 100), description: String(item.description || "").trim().slice(0, 180) })) : (Array.isArray(input.contents) ? input.contents.map((name) => ({ name: plain(name, 100), description: "" })) : []);
  ...
  if (!CATEGORIES.includes(input.category) || !MENU_TYPES.includes(input.mealType) || (kind === "dish" && input.mealType !== "Single Dish") || (kind === "thali" && input.mealType === "Single Dish") || !Number.isInteger(price) || price < 1 || price > 10000 || !Number.isInteger(stockLimit) || stockLimit < 0 || stockLimit > 10000 || !slots.length || slots.some((slot) => !["Lunch", "Dinner"].includes(slot)) || !days.length || days.some((day) => !Number.isInteger(day) || day < 0 || day > 6) || badges.some((badge) => !BADGES.includes(badge))) throw new Error("INVALID_MEAL");
