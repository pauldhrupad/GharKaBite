...
import Meal, { BADGES, CATEGORIES, MEAL_TYPES, MENU_TYPES } from "@/models/Meal";
...
export function normalizeMeal(input) {
  ...
  const kind = input.kind || "thali";
  if (!["thali", "dish"].includes(kind)) throw new Error("INVALID_MEAL");
  ...
  const preparation = (entry, defaultUnit = "portion") => {
    ...
    const quantity = legacyCount ? Number(countedRoti[1]) : entry.preparationQuantity == null ? roti ? 4 : 1 : Number(entry.preparationQuantity);
    const unit = String(legacyCount ? "pieces" : entry.preparationUnit || (roti || countedRoti ? "pieces" : defaultUnit)).trim();
    if (!Number.isFinite(quantity) || quantity <= 0 || quantity > 100 || !unit || unit.length > 30 || /<[^>]*>/.test(unit)) throw new Error("INVALID_MEAL");
    return { preparationQuantity: quantity, preparationUnit: unit };
  };
  const fixedItems = kind === "dish" ? [{ name, description: "", ...preparation(input.fixedItems?.[0] || {}) }] : Array.isArray(input.fixedItems) ? input.fixedItems.map((item) => ({ name: plain(item.name, 100), description: String(item.description || "").trim().slice(0, 180), ...preparation(item) })) : (Array.isArray(input.contents) ? input.contents.map((name) => ({ name: plain(name, 100), description: "", ...preparation({ name }) })) : []);
  ...
  const choiceGroups = Array.isArray(input.choiceGroups) ? input.choiceGroups.map((group) => {
    const options = Array.isArray(group.options) ? group.options.map((option) => ({ id: id(option.id), name: plain(option.name, 100), description: String(option.description || "").trim().slice(0, 180), priceAdjustment: priceValue(option.priceAdjustment ?? 0), active: option.active !== false, ...preparation(option) })) : [];
  ...
  const addOns = Array.isArray(input.addOns) ? input.addOns.map((item) => ({ id: id(item.id), name: plain(item.name, 100), description: String(item.description || "").trim().slice(0, 180), price: priceValue(item.price), active: item.active !== false, maxQuantity: priceValue(item.maxQuantity ?? 1, 20), ...preparation(item, "piece") })) : [];
  ...
  const badges = Array.isArray(input.badges) ? [...new Set(input.badges)].filter((badge) => badge !== "Limited") : [];
  if (!CATEGORIES.includes(input.category) || !MENU_TYPES.includes(input.mealType) || (kind === "dish" && input.mealType !== "Single Dish") || (kind === "thali" && input.mealType === "Single Dish") || !Number.isInteger(price) || price < 1 || price > 10000 || !slots.length || slots.some((slot) => !["Lunch", "Dinner"].includes(slot)) || !days.length || days.some((day) => !Number.isInteger(day) || day < 0 || day > 6) || badges.some((badge) => !BADGES.includes(badge))) throw new Error("INVALID_MEAL");
...
export async function getMenu(date = kolkataDate()) {
  ...
  return meals.map((meal) => {
    ...
    return { id: meal.slug, kind: meal.kind || "thali", name: meal.name, shortDescription: meal.shortDescription,
      ...
      image: meal.image, badges: (meal.badges || []).filter((badge) => badge !== "Limited"), featured: meal.featured,
      choiceUnavailable, available: Boolean(available && !daily?.soldOut && !choiceUnavailable) };
  });
}
