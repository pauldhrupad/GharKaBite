...
async function verifyMenuAvailability(meal, date, session) {
  const daily = await DailyMenu.findOne({ meal: meal._id, date }).session(session).lean();
...
export async function createOrder(userId, body, paymentMethod, { verifyDelivery = checkDelivery } = {}) {
  ...
  try {
    ...
    await session.withTransaction(async () => {
      ...
      if (settings.acceptingOrders === false) throw new CheckoutError("Ordering is temporarily paused by the kitchen.", 409);
      if (payload.period === "Lunch" && settings.lunchEnabled === false || payload.period === "Dinner" && settings.dinnerEnabled === false) throw new CheckoutError(`${payload.period} ordering is currently unavailable.`, 409);
      if (periodCutoffPassed(payload.date, payload.period, settings)) throw new CheckoutError("Ordering for this meal period has just closed. Please choose another available meal period.", 409);
      ...
      for (const item of payload.items) {
        ...
        await verifyMenuAvailability(meal, payload.date, session);
