...
import { calculateDeliveryFee, validatePromoCode } from "./cart-pricing";
...
function normalizePayload(body) {
  ...
  const cleanItems = items.map((item) => ({ mealId: String(item.mealId || ""), quantity: Number(item.quantity) }));
  if (cleanItems.some((item) => !/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(item.mealId) || !Number.isInteger(item.quantity) || item.quantity < 1 || item.quantity > 10) || new Set(cleanItems.map((item) => item.mealId)).size !== cleanItems.length) throw new CheckoutError("Your cart is not valid.");
  ...
  return { date, period, items: cleanItems, contact, address, deliverySlot: body.deliverySlot,
    ...
    promoCode: String(body.promoCode || "").slice(0, 30), notes: String(body.notes || "").trim().slice(0, 300), checkoutKey };
}

async function reserveStock(meal, date, quantity, session) {
  let daily = await DailyMenu.findOneAndUpdate({ meal: meal._id, date }, { $setOnInsert: { meal: meal._id, date, availableOverride: null, soldOut: false, remaining: meal.stockLimit, stockLimitSnapshot: meal.stockLimit } }, { upsert: true, new: true, session });
  ...
  daily = await DailyMenu.findOneAndUpdate({ _id: daily._id, remaining: { $gte: quantity }, soldOut: false }, { $inc: { remaining: -quantity } }, { new: true, session });
...
async function reserveCapacity(date, period, settings, session) {
  ...
  const doc = await DailyCapacity.findOneAndUpdate({ date }, { $setOnInsert: { date, daily: 0, lunch: 0, dinner: 0 } }, { upsert: true, new: true, session });
  const updated = await DailyCapacity.findOneAndUpdate({ _id: doc._id, daily: { $lt: Number(settings.dailyMaximum) }, [field]: { $lt: max } }, { $inc: { daily: 1, [field]: 1 } }, { new: true, session });
...
export async function createOrder(userId, body, paymentMethod, { verifyDelivery = checkDelivery } = {}) {
  ...
  if (!["COD", "DEMO"].includes(paymentMethod)) throw new CheckoutError("Payment method is invalid.");
  await seedCatalog();
  ...
  try {
    ...
    await session.withTransaction(async () => {
      ...
      const meals = await Meal.find({ slug: { $in: payload.items.map((item) => item.mealId) }, active: true, archivedAt: null }).session(session);
      if (meals.length !== payload.items.length) throw new CheckoutError("A meal is no longer available.", 409);
      ...
      for (const item of payload.items) {
        ...
        pricedItems.push({ mealId: meal.slug, name: meal.name, image: meal.image, price: meal.price, quantity: item.quantity, category: meal.category });
      }
      ...
      if (payload.subscriptionId) {
        ...
        if (!subscription || !covered || !payload.items.some((item) => item.mealId === covered.slug) || !subscription.allowedMealTypes.includes(covered.mealType) || (subscription.mode !== "Mixed" && subscription.mode !== payload.period)) throw new CheckoutError("This plan cannot cover the selected meal.", 409);
      }
      const subtotal = pricedItems.reduce((sum, item) => sum + item.price * item.quantity, 0);
      const payableSubtotal = subtotal - (covered?.price || 0);
      const promo = payload.promoCode ? validatePromoCode(payload.promoCode, payableSubtotal) : null;
      if (payload.promoCode && !promo.valid) throw new CheckoutError(promo.message);
      const discount = promo?.discount || 0;
      const deliveryFee = subscription ? 0 : calculateDeliveryFee(subtotal);
      ...
      const [order] = await Order.create([{ orderNumber: `GKB-${payload.date.replaceAll("-", "")}-${crypto.randomBytes(4).toString("hex").toUpperCase()}`, user: userId,
        ...
        paymentMethod, paymentStatus: paymentMethod === "DEMO" ? "paid" : "pending", orderStatus: "received",
        subtotal, deliveryFee, discount: discount + (covered?.price || 0), total,
        notes: payload.notes, statusHistory: [{ status: "received", timestamp: now }] }], { session });
      if (subscription) {
        const updated = await Subscription.findOneAndUpdate({ _id: subscription._id, status: "active", remainingMeals: { $gt: 0 } }, { $inc: { remainingMeals: -1 }, $push: { usageHistory: { order: order._id, mealName: covered.name, usedAt: now } } }, { new: true, session });
...
export async function updateOrderStatus(orderNumber, status) {
  ...
  try {
    ...
    await session.withTransaction(async () => {
      ...
      if (status === "cancelled" && order.serviceDate && !order.reservationsReturned && ["received", "confirmed"].includes(order.orderStatus)) {
        for (const item of order.items) {
          const meal = await Meal.findOne({ slug: item.mealId }).session(session);
          if (meal) await DailyMenu.updateOne({ meal: meal._id, date: order.serviceDate }, { $inc: { remaining: item.quantity } }, { session });
        }
