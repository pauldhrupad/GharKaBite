...
function normalizePayload(body) {
  ...
  if (cleanItems.some((item) => !item.selectedChoices || typeof item.selectedChoices !== "object" || Array.isArray(item.selectedChoices) || !item.selectedAddOns || typeof item.selectedAddOns !== "object" || Array.isArray(item.selectedAddOns))) throw new CheckoutError("Review your Thali customization.");
...
export async function createOrder(userId, body, paymentMethod, { verifyDelivery = checkDelivery } = {}) {
  ...
  try {
    ...
    await session.withTransaction(async () => {
      ...
      if (meals.length !== uniqueSlugs.length) throw new CheckoutError("A Thali is no longer available.", 409);
      ...
      for (const item of payload.items) {
        ...
        pricedItems.push({ mealId: meal.slug, name: meal.name, image: meal.image, price: priced.unitTotal, basePrice: priced.basePrice, fixedItems: priced.fixedItems, selectedChoices: priced.selectedChoices, selectedAddOns: priced.selectedAddOns, quantity: item.quantity, category: meal.category });
