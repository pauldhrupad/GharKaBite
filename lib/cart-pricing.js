...
export function calculatePromoDiscount(promotion, subtotal, now = new Date()) {
  ...
  if (!promotion || !promotion.active || (promotion.expiresAt && new Date(promotion.expiresAt) <= now)) return { valid: false, code, discount: 0, message: "That promo code is not available." };
