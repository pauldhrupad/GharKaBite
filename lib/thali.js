...
export function calculateThaliPrice(thali, choices = {}, addOns = {}, quantity = 1) {
  ...
  for (const group of groups) {
    ...
    const options = ids.map((id) => {
      ...
      if (!option || option.active === false || (option.stock != null && option.stock < quantity)) throw new Error(`${group.name} has an unavailable selection. Please review your Thali.`);
      ...
      return { id: option.id, name: option.name, priceAdjustment };
    });
  ...
  for (const [id, rawQuantity] of Object.entries(addOns)) {
    ...
    if (!addOn || addOn.active === false || count > Number(addOn.maxQuantity ?? 1) || (addOn.stock != null && addOn.stock < count * quantity)) throw new Error(`${addOn?.name || "An add-on"} is unavailable in that quantity.`);
    ...
    selectedAddOns.push({ id, name: addOn.name, price, quantity: count });
  }
  ...
  return { basePrice, choiceAdjustments, addOnTotal, unitTotal, lineTotal: unitTotal * quantity,
    fixedItems: (asArray(thali.fixedItems).length ? thali.fixedItems : asArray(thali.contents).map((name) => ({ name }))).map((entry) => ({ name: entry.name, description: entry.description || "" })),
    selectedChoices, selectedAddOns };
}

export function hasRequiredUnavailable(thali, quantity = 1) {
  return asArray(thali.choiceGroups).some((group) => Number(group.minSelections ?? (group.required ? 1 : 0)) > 0 && asArray(group.options).filter((option) => option.active !== false && (option.stock == null || option.stock >= quantity)).length < Number(group.minSelections ?? 1));
}
