...
export const BADGES = ["Popular", "Low Oil", "Limited", "Today's Special"]; // Limited is legacy-only.
...
const mealSchema = new mongoose.Schema({
  ...
  fixedItems: { type: [{ name: { type: String, required: true }, description: { type: String, default: "" }, preparationQuantity: { type: Number, min: 0.01, default: function () { return /^rotis?$/i.test(this.name) ? 4 : 1; } }, preparationUnit: { type: String, default: function () { return /^rotis?$/i.test(this.name) ? "pieces" : "portion"; } } }], default: [], _id: false },
  choiceGroups: { type: [{
    ...
    options: { type: [{ id: { type: String, required: true }, name: { type: String, required: true }, description: { type: String, default: "" }, priceAdjustment: { type: Number, min: 0, default: 0 }, active: { type: Boolean, default: true }, stock: { type: Number, min: 0, default: null }, preparationQuantity: { type: Number, min: 0.01, default: function () { return /^rotis?$/i.test(this.name) ? 4 : 1; } }, preparationUnit: { type: String, default: function () { return /^rotis?$/i.test(this.name) ? "pieces" : "portion"; } } }], default: [], _id: false },
  }], default: [], _id: false },
  addOns: { type: [{ id: { type: String, required: true }, name: { type: String, required: true }, description: { type: String, default: "" }, price: { type: Number, min: 0, required: true }, active: { type: Boolean, default: true }, maxQuantity: { type: Number, min: 1, max: 20, default: 1 }, stock: { type: Number, min: 0, default: null }, preparationQuantity: { type: Number, min: 0.01, default: 1 }, preparationUnit: { type: String, default: "piece" } }], default: [], _id: false },
  ...
  stockLimit: { type: Number, min: 0, max: 10000 }, // Deprecated; retained for old documents.
